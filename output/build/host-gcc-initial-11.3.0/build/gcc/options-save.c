		       ...
		       struct gcc_options *opts_set)
{
  ...
  if (opts_set->x_prefer_vector_width_type) mask |= HOST_WIDE_INT_1U << 12;
  if (opts_set->x_flag_fentry) mask |= HOST_WIDE_INT_1U << 13;
  if (opts_set->x_ix86_needed) mask |= HOST_WIDE_INT_1U << 14;
  if (opts_set->x_ix86_stack_protector_guard_offset_str) mask |= HOST_WIDE_INT_1U << 15;
  if (opts_set->x_ix86_stack_protector_guard_reg_str) mask |= HOST_WIDE_INT_1U << 16;
  if (opts_set->x_ix86_stack_protector_guard_symbol_str) mask |= HOST_WIDE_INT_1U << 17;
  ptr->explicit_mask[0] = mask;
}
